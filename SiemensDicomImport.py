###########################################################################################
# SiemensDicomImport.py                                                                   #
# 01/03/17                                                                                #
# andrew.robinson@npl.co.uk                                                               #
#                                                                                         #
# Import Siemen's PET data into Mirdad system                                             #
# Based on GeDicomImport.py                                                               #
#                                                                                         #
# NB: THIS IS A WORK IN PROGRESS !!!!!!!                                                  #
#                                                                                         #
# Usage:                                                                                  #
#  python SiemensDicomImport -h for full list of options)                                 #
#                                                                                         #
# Requirements:                                                                           #
...
def readInterFileHeader(inputFileName):
    ...
    for line in inFile:
        ...
        if re.search("matrix size\[2\]", line):
            ...
            words = re.split("=", line)
            y =  words[1]
        if re.search("matrix size\[3\]", line):
            line = line.rstrip('\n\r')
            words = re.split("=", line)
            z =  words[1]
...
def readInterFileData(dataFileName, PixelDims, dtype):
    ...
    ArrayRaw3D = ArrayRaw.reshape(PixelDims, order='F') # Reshape the array into 3D
    
    #ArrayRaw3D = ArrayRaw3D[:,:,::-1]                        # Flip the array in Z
    #ArrayRaw3D = numpy.rot90(ArrayRaw3D)                     # Rotate 90deg CCW

    #np.set_printoptions(threshold=np.inf)
    #print np.nonzero(ArrayRaw3D)
    #print ArrayRaw3D.max()
    #print ArrayRaw3D.min()
    #print ArrayRaw3D.mean()
    
    return ArrayRaw3D.astype(np.uint16)
#----------------------
...
def readIFmatrix(filename):
    ...
    abc = readInterFileData(datafilename, dimensions, dtype)
    print np.nonzero(abc)
    print "------------------"
    print abc.max()
    print abc.min()
    print abc.mean()
...
def changeDataName(datasetname, uid_offset=10):
    ...
    # Change datset name
    ds[0x08,0x103e].value = datasetname
    ds[0x10,0x10].value = datasetname
...
def changePixelData(interfile,sliceOutput):

    print "\n|Pixel Data|"
    newPixelData = readIFmatrix(interfile)

    # Get a slice from 3D array
    #in_array[0,:,:]

    # Swap the array data
    ds.PixelData = newPixelData[:,:,int(sliceOutput)-1].tostring() # Have to write as raw data
...
parser.add_argument("-s", "--sliceOutput", help="Specify slice to write")
...
if args.interfile:
    changePixelData(args.interfile, args.sliceOutput)

# Save file
of = args.outputfile + "_" + str(int(args.sliceOutput) + 1) + ".dcm"
print "\nSaving file: " + of
ds.save_as(of)
#---------------------------------------------------
